...
for(i in unique(train_raw$item_id)) {
  ...
  off_dat_i <- preds_time_month_wday_snap_event %>% filter(item_id == i)
  prices_i <- prices %>% 
    filter(item_id == i) %>% 
    pivot_longer(starts_with("X"), 
                 ...
                 values_to = "price_z", 
                 values_drop_na = T)
  price_mod_data_i <- prep_time_data(dat_i, 
                                    off_data = off_dat_i, 
  ...
  # non0 model
  non0_mod_data <- price_mod_data_i %>% 
  ...
  # sales modes
  sales_mod_data <- price_mod_data_i %>% filter(sales > 0, between(lp_sales_base, -7, 7))
  cf_sales <- if(nrow(sales_mod_data) > 2) {
    m_sales_time_i <- glm(I(sales != 0) ~ price_z, 
                          sales_mod_data, 
                          family = poisson(), 
